...
struct FastBitset : public _Base_bitset<Words> {
    const static int bit = sizeof(unsigned long);
    FastBitset(unsigned long val = 0) 
        : _Base_bitset<Words>(val) 
    {
    }
    void insert(size_t index) {
        (this->_M_w[this->_S_whichword(index)]) |= (1UL << (this->_S_whichbit(index)));
    }
    void flip(size_t index) {
        (this->_M_w[this->_S_whichword(index)]) ^= (1UL << (this->_S_whichbit(index)));
    }
    bool contains(size_t index) const {
        return (this->_M_w[this->_S_whichword(index)] >> (this->_S_whichbit(index))) & 1;
    }
    size_t upper_bound(size_t index, size_t border = Words * (1 << bit)) const {
        return this->_M_do_find_next(index, border);
    }
    size_t lower_bound(size_t index, size_t border = Words * (1 << bit)) const {
        if (contains(index)) {
            return index;
        }
    ...
    size_t begin() const {
        return this->_M_do_find_first(Words * (1 << bit));
    }
    unsigned int GetWordByIndex(size_t index) {
        return this->_M_w[index >> bit];
    }
};
...
struct FastSet {
    ...
    void erase(size_t index) {
        if (contains(index)) {
            ...
            if (table0.GetWordByIndex(index) == 0UL
                    && (table1.GetWordByIndex(index >> bit) ^ (index >> bit)) == 0UL) {
                table1.flip(index >> bit);

                if (table1.GetWordByIndex(index >> bit) == 0UL
                        && (table2.GetWordByIndex(index >> bit2) ^ (index >> bit2)) == 0UL) {
                    table2.flip(index >> bit2);
                    
                    if (table2.GetWordByIndex(index >> bit2) == 0UL
                            && (table3.GetWordByIndex(index >> bit3) ^ (index >> bit3)) == 0UL) {
    ...
    size_t lower_bound1(size_t index, size_t border) {
        ...
        if (current_index > (index >> bit)) return lower_bound0(current_index << bit, border);

        size_t current_border = (1 + (index >> bit)) << bit;
...
int main() {
    ...
    
    {
        ...
        size_t sum = 0;
        for (int i = 0; i < n * 0.5; i++) {
            auto it = st1.lower_bound(data[i]);
            sum += it == st1.end() ? n : *it;
            st1.insert(data[i]);
            st1.erase(rem[i]);
